...
def connect_and_store_socket(port, protocol):
    try:
        ...
        logger.info(f"🔗 Connected to http://{SERVER_IP}:{port} ({protocol})")
        return port, s
    except Exception as e:
        logger.error(f"❌ Connection failed to http://{SERVER_IP}:{port} - {e}")
        return port, None
            ...
            for key in keys:
                ...
                while redis_client.llen(key) > 0:
                    ...
                    if session_id in session_sockets:
                        try:
                            session_sockets[session_id].sendall(bytes.fromhex(hex_data))
                            logger.info(f"📤 Sent to {SERVER_IP}:{port} ({protocol}) for session {session_id}: {hex_data}")
                            # Delete Redis data after successfully sending it
                            redis_client.delete(key)
                            logger.info(f"✅ Deleted Redis key {key} after sending data.")
                        except Exception as e:
                            logger.error(f"❌ Failed to send for session {session_id}: {e}")
                            session_sockets[session_id].close()
                            del session_sockets[session_id]
